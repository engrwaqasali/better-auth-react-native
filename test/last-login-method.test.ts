...
describe('last-login-method react-native', async () => {
  ...
  it('should check if a specific method was last used', async () => {
    const storage = createMockStorage();
    ...
    expect(client.isLastUsedLoginMethod('email')).toBe(true);
    expect(client.isLastUsedLoginMethod('google')).toBe(false);
  });
});
